...
# seperti hero banner). Untuk teks & background utama, kita pakai CSS variable
# bawaan Streamlit (var(--...)) supaya otomatis ikut tema Light/Dark/Auto.
PRIMARY, PRIMARY_DARK = "#0B5FA5", "#073B69"
ACCENT = "#2E86DE"

# CSS kustom tampilan website — mengikuti tema Streamlit yang aktif
st.markdown(f"""
<style>
    html, body, [class*="css"] {{ font-family: 'Segoe UI', 'Inter', sans-serif; }}

    /* Sidebar sengaja dibuat gelap permanen (brand color) di kedua tema.
       Hanya label/judul statis yang dipaksa putih - widget (selectbox, radio)
       DIBIARKAN pakai warna bawaan Streamlit, karena widget itu sudah otomatis
       menyesuaikan diri (kotak+teks kontras) sesuai tema Light/Dark yang aktif.
       Kalau ikut dipaksa putih, teksnya malah bentrok saat kotaknya terang. */
    ...
    /* Dropdown pilihan (Select Page / Pilih Provinsi) dirender di luar sidebar
       (sebagai popover), jadi harus distyling terpisah agar tetap terbaca */
    ...
    /* Hero banner selalu gradasi biru dengan teks putih, di kedua tema */
    .hero {{
